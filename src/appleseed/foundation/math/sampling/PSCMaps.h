...
T PSCMaps<T>::eval_Ap_inverse( T Ap_value ) const
{
   ...
   auto Ap_func      { [=]( T y ) { return eval_Ap( y )/Ap_max_value;       } } ;
   auto Ap_integrand { [=]( T y ) { return eval_par_integrand( y )/Ap_max_value ; } } ;
...
void PSCMaps<T>::hor_map( T s, T t, T &x, T &y ) const
{
   ...
   const T u        = y_is_neg ? T(1.0)-T(2.0)*t
                                   : T(2.0)*t - T(1.0) ;
...
T PSCMaps<T>::eval_Ar_inverse( T Ar_value ) const
{
   ...
   auto Ar_func      { [=]( T theta ) { return eval_Ar( theta )/Ar_max_value;       } } ;
   auto Ar_integrand { [=]( T theta ) { return eval_rad_integrand( theta )/Ar_max_value ; } } ;
...
void PSCMaps<T>::rad_map( T s, T t, T &x, T &y ) const
{
   ...
   const T u            = angle_is_neg ? T(1.0)-T(2.0)*t
                                           : T(2.0)*t - T(1.0) ;
